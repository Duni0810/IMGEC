...
void SendAUXData(BYTE PortNum)  // Dino 0907             
{
    ...
    {	
        if(ECSendtoAUXFlag)                 // If this byte is EC scan channel ACK, no send to host
        {
...
void RAM_Send2Port(BYTE p_PortNum, BYTE p_cmd)
{
    if(p_PortNum==0x00)
    {
        PSCTL1 = 0x1D;
...
// 
//-----------------------------------------------------------------
void ClearPS2IFPendingEvent(void)
{
    InhibitAUXDevice();
        ...
        for(index=0x00;index<bytecunt;index++)
        {
            *asPS2Struct[PortNum].ps2ctrl=PS2_ReceiveMode;  // Get Last ACK by bytecunt
			...
			if(Oem_RTP_ID_CMD == 0)
			{
				...
				switch(Oem_RTP_ID_Step&0x7f)
				{
					case Oem_RTP_ID_Elan_step:
						if(PS2IFAck == 0x3c)
						{
							guoyong003 = 0x88;
							
							Oem_RTP_ID_CMD = Elan_TP_ID;
							SET_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_SUCCESS);						
						}
							
						break;
						
					case Oem_RTP_ID_SY_step:
						if(PS2IFAck == 0x47)
						{

							guoyong003 = 0x77;
							Oem_RTP_ID_CMD = Synaptics_TP_ID;
							SET_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_SUCCESS);	
						}

						break;

					case Oem_RTP_ID_ALPS_step:
						if(PS2IFAck == 0x73)
						{
							guoyong003 = 0x66;
							Oem_RTP_ID_CMD = ALPS_TP_ID;
							SET_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_SUCCESS);		
						}
...
{	 
    ...
    for(index=0x00;index<3;index++)
    {
        if(IS_MASK_SET(AuxFlags[index],(DEVICE_IS_KEYBOARD+DEVICE_IS_MOUSE)))   // keyboard or mouse
        {
            //*asPS2Struct[index].ps2ctrl=PS2_ReceiveMode;
            //SET_MASK(*asPS2Struct[index].ps2ier, asPS2Struct[index].ctrlbit); 
...
void Read_TP_ID(void)
{
	...
	if(Oem_RTP_ID_1s_EN == 0x01)
		Oem_RTP_ID_1s_T = Oem_RTP_ID_1s_T + 1;


	if(Oem_RTP_ID_1s_T<1) 
	{
		return;
	}
	else
		Oem_RTP_ID_1s_EN = 0x00;

	while(Oem_RTP_ID_LOOP_Index <= 3)
	{
	
	if(IS_MASK_CLEAR(Oem_RTP_ID_Step,Oem_RTP_ID_SUCCESS))
		{

//^^^^^^^^^^^^^^^^^^^^^^^^^^^ELAN TP^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
			RamDebug(0x66);
			SET_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_Elan_step);

			data_pntr = ELAN_ID_SQ;
			for(index_cmd=0;index_cmd<(sizeof(ELAN_ID_SQ)/2);index_cmd++)
			{

				Send2PortNWait(2,*data_pntr++,*data_pntr++);
				
			}
			CLEAR_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_Elan_step);

			RamDebug(0x66);

			if(IS_MASK_SET(Oem_RTP_ID_Step,Oem_RTP_ID_SUCCESS))
			{
				break;
			}
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^Synaptics TP^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
			...
			RamDebug(0x44);
			SET_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_SY_step);

			data_pntr = SYN_ID_SQ;
			for(index_cmd=0;index_cmd<(sizeof(SYN_ID_SQ)/2);index_cmd++)
			{
				Send2PortNWait(2,*(data_pntr++),*(data_pntr++));

				
			}
			CLEAR_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_SY_step);
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^ALPS TP^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
			...
			RamDebug(0x33);
			SET_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_ALPS_step);
		
			data_pntr = ALPS_ID_SQ;
			for(index_cmd=0;index_cmd<(sizeof(ALPS_ID_SQ)/2);index_cmd++)
			{
					
				Send2PortNWait(2,*data_pntr++,*data_pntr++);		
				
			}
			CLEAR_MASK(Oem_RTP_ID_Step,Oem_RTP_ID_ALPS_step);
