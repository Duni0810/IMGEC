...
//-----------------------------------------------------------------------
// Process command data 97 -TP driver report to EC for touchpad disable/ enable 
//-----------------------------------------------------------------------
void Cmd_97Data(void)
{

	if((KBHIData & 0x0f) ==0x02)
	{			
		Device_Status_Lenovo |= TouchPad_S_B;
		Oem_Device_Status |= TouchPad_IN;
	}
	else if((KBHIData & 0x0f) ==0x01)
	{				
		Device_Status_Lenovo &= ~TouchPad_S_B;	
		Oem_Device_Status &= ~ TouchPad_IN;
	}
...
//*****************************************************************************
//
// Short delay after write data to output register of 64h port.
//
//  parameter :
//      none
//
