...
BYTE i2c_read_reg(BYTE i2c_addr, BYTE reg)
{
    ...
    {
        ret = 0;
        ret = I2C_ReadStream(_SMB_TYPE_C,
                            i2c_addr,
                            &SPIBuffer[0],
                            2);
    }
...
/*-----------------------------------------------------------------------------
 * @subroutine - i2c_write_block_reg
 * @function - i2c_write_block_reg
 * @Upstream - By call
 * @input    - None
 * @return   - None
 * @note     - None
...
void i2c_write_block_reg(BYTE i2c_addr, BYTE reg, BYTE length, BYTE *buf)
{
    BYTE    i;
