...
//----------------------------------------------------------------------------
// Read thermal sensor fail
//----------------------------------------------------------------------------
void Error_PollThermal(void)
{
...
const sSCS code asSCS1[] = 
{
	...
	{ Cmd_ReadWord, 	SmartBat_Addr,		BATCmd_DCap,		&BAT1_DScap_L, 				B1_DScap,		&SMBus1Flag2,	BAT1_Read_OK,	BAT1_Read_Fail, 0x00	},		
	{ Cmd_ReadWord, 	SmartBat_Addr,		BATCmd_SerialNo,	&BAT1_SerialNumber,			B1_SerialNo,	&SMBus1Flag5,	BAT1_Read_OK,	BAT1_Read_Fail, 0x00	},		
...
void SMBusCenterSelection(BYTE channel) 
{
	...
	{
        //if(*asSSC[channel].serviceflag !=0x00)
        //{
...
/*-----------------------------------------------------------------------------
 * @subroutine - Service_HI2C_Interface
 * @function - Service_HI2C_Interface
 * @upstream - By call
 * @input    - None
 * @return   - None
 * @note     - None
...
void Service_HI2C_Interface(void)
{
    if (xHI2C_CHN > 0)
    {
        /* Currently, only support channel-0 */
        return;
    }
#if 1   /* For Analogix Update Firmware */
    if (xHI2C_START == 0x01)
    {
        xHI2C_DATA0 = i2c_read_reg(xHI2C_ADDR, xHI2C_CMD);
        xHI2C_STATUS = 0;   // SET NOT BUSY
        xHI2C_START = 0;    // SET FINISHED
        return;
    }
    if (xHI2C_START == 0x03)
    {
        i2c_read_block_reg(xHI2C_ADDR, 
                            xHI2C_CMD, 
                            xHI2C_BYTECNT, 
                            &xHI2C_DATA[0]);
        xHI2C_STATUS = 0;   // SET NOT BUSY
    ...
    if (xHI2C_START == 0x81)
    {
        i2c_write_reg(xHI2C_ADDR, xHI2C_CMD, xHI2C_DATA0); 
        xHI2C_STATUS = 0;   // SET NOT BUSY
        xHI2C_START = 0;    // SET FINISHED
        return;  
    }
     if (xHI2C_START == 0x83)
    {
        i2c_write_block_reg_b(xHI2C_ADDR, 
                            xHI2C_CMD, 
                            xHI2C_BYTECNT, 
                            &xHI2C_DATA[0]);
        xHI2C_STATUS = 0;   // SET NOT BUSY
